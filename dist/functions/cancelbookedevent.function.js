export class CancelBookedEventFunction {
    constructor() {
        this.name = "cancelBookedEvent";
        this.description = "Cancel the reservation for a customer of an event at Home of Zen." +
    ...
    async handleExecution(request) {
        ...
        const result = await this.cancelBookedEvent(email, date, time);
        return result;
    }
    async cancelBookedEvent(email, date, time) {
        const body = {
            email: email,
            date: date,
            time: time
