...
export class CancelBookedLessonFunction implements McpFunction {
    ...
    public async handleExecution(request: any) {
        ...
        const result = await this.cancelBookedLesson(email as string, date as string, time as string);
        return result;
    }

    private async cancelBookedLesson(email: string, date: string, time: string): Promise<any> {
        const body = {
            email: email,
            date: date,
            time: time
